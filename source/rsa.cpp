...
long long ExtEuclid(long long a, long long b)
{
 ...
 while (a!=0) {
   q = gcd/a; r = gcd % a;
   m = x-(__int128)u*q; n = y-(__int128)v*q;
...
long long rsa_modExp(long long b, long long e, long long m)
{
  ...
  if(e == 1) return b;
  if( e % 2 == 0){
    return ( rsa_modExp((__int128)b * b % m, e/2, m) % m );
  }
  if( e % 2 == 1){
    return ((__int128)b * rsa_modExp(b, (e-1), m) % m );
  }
                  ...
                  const struct private_key_class *priv)
{
  ...
  // (decrypted)
  char *decrypted = (char *)malloc(message_size);
